...
from adversial_search.board import Board, Move, Piece
...
class TTTPiece(Piece , Enum):
    ...
    def opposite(self) -> TTTPiece:
        if self == TTTPiece.X: return TTTPiece.O
        elif self == TTTPiece.O: return TTTPiece.X
        else: return TTTPiece.E
...
class TTTBoard(Board):
    ...
    def evaluate(self, player: Piece) -> float:
        if self.is_win and self.turn ==player: return -1
        elif self.is_win and self.turn != player: return 1
        else: return 0
